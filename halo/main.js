...
function generateShortSlug(title) {
  ...
  const originalString = `${title}${process.pid}${salt}${Date.now()}`;
...
var HaloSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    .setName("博客文件夹路径")
    .setDesc("设置用于存放博客软链接的文件夹路径")
    .addText((text) => {
      text
        .setPlaceholder("例如: blogs")
...
var HaloService = class {
  ...
  async publishPost() {
    ...
    // 处理摘要
    if (matterData?.excerpt !== undefined) {
      params.spec.excerpt = {
        raw: matterData.excerpt || '',
        autoGenerate: matterData.excerpt === ''
      };
    }
    ...
    // 处理可见性
    if (matterData?.visible) {
      params.spec.visible = matterData.visible;
    }
    
      // 修改 slug 处理
    if (!matterData?.slug || matterData.slug.includes('chuan-tou')) {
      params.spec.slug = generateShortSlug(params.spec.title || activeEditor.file.basename);
    } else {
      params.spec.slug = matterData.slug;
    }

    // 修改 publishTime 处理
    if (matterData?.publishTime) {
      // 如果有设置时间，转换为 ISO 格式
      const [_, year, month, day, hour, minute, second] = matterData.publishTime.match(/^(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2}):(\d{2})$/);
      const date = new Date(year, month - 1, day, hour, minute, second);
      params.spec.publishTime = date.toISOString();
    } else if (!params.metadata.name) {  // 首次发布且没有设置时间时
      // 使用当前时间
      params.spec.publishTime = new Date().toISOString();
    }
    // 处理分类
    ...
    try {
// 在 publishPost 方法中修改 try-catch 块
      if (params.metadata.name) {
        ...
        // 修改这行，使用 generateShortSlug 替代 slugify
        params.spec.slug = generateShortSlug(params.spec.title);
    ...
    this.app.fileManager.processFrontMatter(activeEditor.file, (frontmatter) => {
      ...
      frontmatter.excerpt = params.spec.excerpt.autoGenerate ? '' : params.spec.excerpt.raw;
      // 格式化 publishTime 显示
      if (params.spec.publishTime) {
        // 从 ISO 格式转换回本地时间显示
        const utcDate = new Date(params.spec.publishTime);
        // 手动添加 8 小时来转换为中国时间
        const localDate = new Date(utcDate.getTime());
        frontmatter.publishTime = localDate.toLocaleString('zh-CN', {
          year: 'numeric',
          month: '2-digit',
          day: '2-digit',
          hour: '2-digit',
      ...
      frontmatter.visible = params.spec.visible || 'PUBLIC';
  ...
  async updatePost() {
    ...
    this.app.fileManager.processFrontMatter(activeEditor.file, (frontmatter) => {
      ...
      frontmatter.excerpt = post.post.spec.excerpt.autoGenerate ? '' : post.post.spec.excerpt.raw;
      frontmatter.publishTime = post.post.spec.publishTime ? 
        new Date(post.post.spec.publishTime)
      ...
      frontmatter.visible = post.post.spec.visible || 'PUBLIC';
      frontmatter.categories = postCategories || [];
      frontmatter.tags = postTags || [];
      frontmatter.halo = {
        site: this.site.url,
...
var HaloPlugin = class extends import_obsidian8.Plugin {
  // 将文件复制到博客文件夹的方法
  async copyToBlogFolder(file) {
    // 检查是否设置了博客文件夹路径
    if (!this.settings.blogFolderPath) {
      new import_obsidian8.Notice("请先在设置中配置博客文件夹路径");
      return;
    }
    ...
    try {
      ...
      // 构建目标路径
      const targetDir = this.app.vault.adapter.getFullPath(this.settings.blogFolderPath);
      const targetPath = require('path').join(targetDir, file.name);
      ...
      if (require('fs').existsSync(targetPath)) {
        new import_obsidian8.Notice(`目标文件 ${file.name} 已存在`);
        return;
      }
      ...
      // 构建双向链接
      const relativePath = this.settings.blogFolderPath + '/' + file.name;
      const backlink = `[[${relativePath}|已复制到博客文件夹]]\n`;
      
      // 在文件开头添加双向链接
      const newContent = backlink + sourceContent;
      
      // 更新源文件
      await this.app.vault.modify(file, newContent);
      
      new import_obsidian8.Notice(`已复制文件到博客文件夹并添加链接: ${file.name}`);
    } catch (error) {
      console.error("复制文件失败:", error);
      new import_obsidian8.Notice(`复制文件失败: ${error.message}`);
    }
  }
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian8.TFile && file.extension === "md") {
          menu.addItem((item) => {
            item
              .setTitle("复制到博客文件夹")  // 修改这里
              .setIcon("copy")  // 也可以将图标从 link 改为 copy
              .onClick(async () => {
                await this.copyToBlogFolder(file);
              });
          });
        }
      })
